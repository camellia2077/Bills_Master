...
def handle_modification():
    ...
    for file_path in files_to_process:
        ...
        # 调用新模块的函数
        result = modify_bill_file(file_path, ENABLE_SUM_UP_LINES, ENABLE_ADD_AUTORENEWAL)
        ...
        if result['error']:
            ...
            for log_entry in result['log']:
                if "更新" in log_entry or "添加" in log_entry or "计算" in log_entry:
                    print(f"{GREEN}  - {log_entry}{RESET}")
                elif "未作修改" in log_entry:
